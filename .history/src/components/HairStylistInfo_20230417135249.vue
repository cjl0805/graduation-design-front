<template>
  <div>
    <div class="div">
      ...
      <el-button type="primary" @click="getHairstyle()">修改信息</el-button>
    </div>
    <el-dialog
      title="修改密码"
    ...
    >
      <el-form :model="infoForm">
        <el-form-item label="个人技能">
          <br />
          <el-checkbox-group v-model="checkedSkill">
            <el-checkbox
              :checked="hairstyle.checked"
              v-for="hairstyle in hairstyles"
              :label="hairstyle"
              :key="hairstyle.id"
              @change="handleCheckedSkillChange"
              >{{ hairstyle.hairstyle }}</el-checkbox
            >
          </el-checkbox-group>
        </el-form-item>
...
export default {
  ...
  methods: {
    load() {
      ...
      this.axios({
        method: "GET",
        url: "http://localhost:8090/graduation/design/hairstyle/list/checked",
        params: {
          username: window.localStorage.getItem("username"),
        },
      })
    ...
    updateInfo() {
      ...
      for (let i = 0; i < this.checkedSkill.length; i++) {
        if (i === this.checkedSkill.length - 1) {
          this.infoForm.skill += this.checkedSkill[i].hairstyle;
        } else {
          this.infoForm.skill += this.checkedSkill[i].hairstyle + ",";
        }
      }
