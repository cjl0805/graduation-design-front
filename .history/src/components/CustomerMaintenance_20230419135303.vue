<template>
  <div class="div11">
    ...
    >
      ...
      <el-table-column prop="identityCard" label="身份证" style="width: 25%">
      </el-table-column>
      <el-table-column prop="password" label="密码" style="width: 25%">
      </el-table-column>
      <el-table-column prop="phone" label="手机号" style="width: 25%">
      </el-table-column>
      <el-table-column prop="img" label="头像" style="width: 25%">
    ...
    >
      <el-form :model="infoForm1">
        <el-form-item label="新的密码">
          <el-input v-model="infoForm1.password1" autocomplete="off"></el-input>
        </el-form-item>
        <el-form-item label="确认密码">
          <el-input v-model="infoForm1.password2" autocomplete="off"></el-input>
        </el-form-item>
      </el-form>
  ...
  methods: {
    ...
    getPage() {
      this.axios({
        method: "GET",
        url:
          "http://localhost:8090/graduation/design/customInfo/get/" +
          this.currentPage +
          "/" +
          this.pageSize,
        params: {
    ...
    deleteRow(index, rows) {
      ...
      this.axios({
        method: "DELETE",
        url: "http://localhost:8090/graduation/design/customInfo/delete",
        data: rows[index],
      }).then((res) => {
        console.log(res.data);
        if (res.data.code === 200) {
    ...
    update() {
      if (this.infoForm1.password1 === this.infoForm1.password2) {
        this.axios({
          method: "PUT",
          url: "http://localhost:8090/graduation/design/user/update/password",
          params: {
            password: this.infoForm1.password1,
