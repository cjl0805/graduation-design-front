<template>
  <div>
    ...
    <div class="info">
      <HairStylistInfo v-if="HairStylistInfo"></HairStylistInfo>
      <HairStylistAppointment v-if="HairStylistAppointment"></HairStylistAppointment>
    </div>
  </div>
</template>

<script>
import HairStylistInfo from "./HairStylistInfo.vue";
import HairStylistAppointment from "./HairStylistAppointment.vue";
export default {
  name: "UserDescription",
  components: {
    HairStylistAppointment,
    HairStylistInfo,
  },
  data() {
    return {
      ...
      file: " ",
      HairStylistInfo: true,
      HairStylistAppointment: false,
    };
  },
  created() {
    this.load();
  ...
  methods: {
    load() {
      this.axios({
        ...
        if (res.data.data != null) {
          ...
          window.localStorage.setItem("hairstylist", res.data.data.name);
        }
      });
    },
    click1() {
      this.HairStylistInfo = true;
      this.HairStylistAppointment = false;
    },
    click2() {
      this.HairStylistInfo = false;
      this.HairStylistAppointment = true;
    },
    ...
    saveImg() {
      this.axios({
        method: "POST",
        url: "http://localhost:8090/graduation/design/hairstylist/save/img",
        params: {
          username: window.localStorage.getItem("username"),
